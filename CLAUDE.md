...
**Visibility rule:** the bottom nav appears **only on the `/shell` route**. Every other route in the GoRouter is a top-level route, so pushing it covers the shell scaffold and the nav is hidden for free — there is no `withNavBar: false` to set. Detail screens, payment, profile sub-pages, maps, etc. are all top-level routes.

**Entry-point pattern.** Every "go home" or "land on tab N" action navigates to the shell route with the tab as a query param: `context.go('/shell?tab=N')` (or `context.goNamed(Routes.shell, queryParameters: {'tab': '$n'})`). `AppShell` reads `initialTab` (and `skipLogin`) from those query params — they are *not* `extra`, precisely so a plain string URL works from anywhere (including `routeFromNotification`). Examples:
- Logout / splash → `context.go('/shell?tab=0')`
- Booking-confirmed "Go to Bookings" → `context.go('/shell?tab=2')`
- Avatar tap from inside the shell → `context.jumpToShellTab(3)` (no navigation)
...
- **`app_router.dart`** — the single `GoRouter appRouter` (wired into `MaterialApp.router` in `bloc_providers.dart`). Every screen is a flat top-level `GoRoute`; `initialLocation: '/'` is `SplashScreenOld`. **Multi-parameter destinations take a typed `*Args` class via `extra:`** — those arg classes (`AuthScreenArgs`, `CarsInformationArgs`, `PaymentMethodsArgs`, `InvoiceArgs`, `LocationPickerArgs`, etc.) are defined at the bottom of `app_router.dart`, colocated so the shape of `extra:` is discoverable in one place. Single-value routes pass the bare value as `extra` (e.g. `/offer-details` takes `int`, `/web-payment` takes `String?`).
- **`routes.dart`** — `class Routes` with a `static const` name per route. Navigate by name: `context.pushNamed(Routes.cashback)`, `context.goNamed(...)`. **Key caveat documented in this file:** `extra` is not serializable, so **no route is reachable by deep link, FCM payload, or app-restart restoration** as-is. The two exceptions threaded as query params (so a plain URL works) are `/shell?tab=N&skipLogin=…`.
- **`notification_router.dart`** — `routeFromNotification(context, type:, data:)` is THE single source of truth mapping a notification `type` → destination (`cashback`→cashback, `booking`→`/shell?tab=2`, `offer`→offer-details/offers, `update`/`unknown`→`/shell?tab=0`). Both the in-app notifications screen and (eventually) FCM taps must route through it. Add new notification destinations here, not at call sites.
...
- **`SplashScreenOld` is the active splash**, not deprecated despite the name. It is the `'/'` route of `appRouter`; source at `lib/modules/auth/splash_screen.dart`. Cross-fades between two states (`gradient1` background → white) over `Darbak_logo.png`, then `context.go('/shell?tab=0')` (returning user) or `context.go('/language')` (first run) based on the `isLanguageSelected` flag in `SharedPreferences`. Landing on `/shell` is how the shell first mounts. (`ComposeUi` no longer exists — that intermediate widget was removed in the go_router migration.)
