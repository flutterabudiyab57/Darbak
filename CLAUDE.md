...
- **`ShellBottomNavBar`** (`bottom_nav_bar.dart`) is a floating, non-curved pill bar (plain `Row` in a `Container` with margins — no notch, no third-party nav package). It is the *only* place the nav appears. Icons are SVGs (`flutter_svg` + `ColorFilter`), not PNGs. `lib/modules/shell/shell_bottom_bar_metrics.dart` is the single shared source for the bar's height/margins and exposes `shellBottomInset(context)` — the bottom clearance root tab screens (`search_Screen.dart`, `profile.dart`, `all_cars_screen.dart`, `running_now.dart`, `previousOrders.dart`) should add as `ListView`/scroll-view bottom padding so content isn't hidden behind the floating bar. `extendBody: true` is set on the shell's `Scaffold` so tab content draws behind the bar.
